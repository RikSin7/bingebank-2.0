...
export default function CastCarousel({ cast }: CastCarouselProps) {
  ...
  return (
    <section className="relative group/cast w-full overflow-hidden">
      <h2 className="text-xl md:text-2xl font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2 px-4 md:px-0">
        ...
        <div ref={scrollRef} className="flex py-2 gap-4 md:gap-8 overflow-x-auto px-4 md:px-8 pb-4 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
          {cast.map((member, index) => (
            ...
            >
              ...
              <p className="text-sm font-bold text-[var(--text-primary)] w-full truncate group-hover/card:text-purple-500 transition-colors">{member.name}</p>
              <p className="text-xs text-purple-600 dark:text-purple-300 w-full truncate mt-1">{member.character}</p>
            </Link>
          ))}
        </div>
      </div>
